...
class SparkExpectationsActions:
        ...
        try:
            if (
                ...
                if SparkExpectationsActions.match_parentheses(_dq_rule["expectation"]):
                    ...
                    SparkExpectationsActions.debug_print(_context, "In action.py file match stmt, query: " + str(match))
                    if match:
                        # function to execute SQL and get the result
                        def execute_sql_and_get_result(
                            _se_context: SparkExpectationsContext, query: str
                        ) -> int:
                            SparkExpectationsActions.debug_print(_context, "In action.py file query stmt, query: " + str(query))
                            return (
                        ...
                        SparkExpectationsActions.debug_print(_context, "In action.py file match stmt match.group(1), query: " + str(match.group(1)))

                        _querydq_target_query_output = match.group(
                            4
                        ) or execute_sql_and_get_result(_context, match.group(5))

                        SparkExpectationsActions.debug_print(_context, "In action.py file match stmt match.group(5), query: " + str(match.group(5)))


                        # assignment of actual_outcome and expected_outcome
                        actual_outcome = _querydq_source_query_output
                        SparkExpectationsActions.debug_print(_context, "In action.py file error value, actual_outcome: " + str(actual_outcome))
                        expected_outcome = (
                            str(match.group(2)) + str(_querydq_target_query_output)
                            if _querydq_target_query_output
                            else None
                        )
                        SparkExpectationsActions.debug_print(_context,"In action.py file _querydq_target_query_output value, expected_outcome: " + str(expected_outcome))
                ...
                if _source_dq_status:
                    ...
                    row_count = 0 # Changed from None to 0

                # if _context.get_dq_rules_params.get("debug", False):
                SparkExpectationsActions.debug_print(_context, "In action.py file, _source_dq_status: " + str(_source_dq_status) + " " + "_target_dq_status: " + str(_target_dq_status) + " " + "row_count: " + str(row_count) + " " + "_query_dq_result: " + str(_query_dq_result))
                ...
                SparkExpectationsActions.debug_print(_context, "In action.py file, actual_row_count: " + str(actual_row_count) + " " + "error_row_count: " + str(error_row_count) + " " + "row_count: " + str(row_count))
                status = "pass" if (abs(error_row_count) <= ((_dq_rule["error_drop_threshold"] / 100) * row_count)) else "fail"
    ...
    def debug_print(_context, *args, **kwargs) -> None:
        caller = inspect.stack()[1][3]
        line_number = inspect.currentframe().f_back.f_lineno
        if _context.get_dq_rules_params.get("debug", False):
            print(f"SE_DEBUG: File: {__file__}, Function: {caller}, Line: {line_number}", *args, **kwargs)